"""
Smart Persistence Model (SPM) helper functions.

This module provides two SPM implementations:
- `spm_pv_forecast_v1`: uses `pvlib` to compute sun angles.
- `spm_pv_forecast_v2`: reference implementation based on empirical formulas.
"""
...
def spm_pv_forecast_v1(
    labels: np.ndarray,
    datetime_labels: Sequence[pd.Timestamp],
    ...
    if parameters is None:
        parameters = _default_parameters_v1()

    smart_predictions = _calculate_predictions(labels, datetime_labels, parameters)
    return np.array(smart_predictions)


def _default_parameters_v1() -> dict[str, Any]:
    # Standard parameters for the SKIPPD dataset (Source: https://doi.org/10.1016/j.solener.2023.03.043)
    return {
        "latitude": 37.42808,
        "longitude": -122.17023,
        "altitude": 23,
...
def _calculate_predictions(
    ...
    smart_predictions: list[np.float32] = []
    for i, ts in enumerate(datetime_labels):
        actual_pv_output = labels[i]
        smart_pred, _, _ = _the_future_pv_output(ts, actual_pv_output, parameters)
        smart_predictions.append(np.float32(smart_pred))
    return smart_predictions


def _the_future_pv_output(
    time: pd.Timestamp,
    ...
    try:
        ...
        future_clr_pv_output = _calculate_clr_pv_output(
            time + pd.Timedelta(value=parameters["time_delta"], unit="minutes"),
            parameters,
        )
...
def _calculate_clr_pv_output(
    ...
    try:
        ...
        pclr_t = (
            parameters["max_solar_irradiance"]
            * parameters["effective_panel_area"]
            * (
                np.cos(panel_elevation_rad) * np.cos(sun_zenith_rad)
                + np.sin(panel_elevation_rad)
                * np.sin(sun_zenith_rad)
...
def _calc_sun_position(
    ...
    try:
        ...
        # NOTE: Preserves original behavior: returns pvlib's "elevation" first.
...
def spm_pv_forecast_v2(
    labels: np.ndarray,
    datetime_labels: Sequence[datetime.datetime],
    parameters: dict[str, Any] | None = None,
) -> np.ndarray:
    """
    ...
    if parameters is None:
        ...
        parameters = {
            ...
            "time_zone_center_longitude": -120,  # time zone correction in degrees
        }

    ntimes = len(datetime_labels)
    per_prediction = np.zeros(ntimes)
    for i in range(ntimes):
        csi_cur, _ = Relative_output(datetime_labels[i], labels[i], parameters)
        _, p_theo_pred = Relative_output(
            datetime_labels[i] + datetime.timedelta(minutes=parameters["time_delta"]),
            labels[i],
            parameters,
        )
...
def Relative_output(
    ...
    # Constants
    p0 = parameters["max_solar_irradiance"] / 1000  # kW/m^2
    a_eff = parameters["effective_panel_area"]  # m^2
    epsilon = math.radians(parameters["panel_elevation"])
    zeta = math.radians(parameters["panel_azimuth"])
    ...
    alpha = 2 * math.pi * (time_of_day - 43200) / 86400  # hour angle (rad)
    delta = math.radians(23.44 * math.sin(math.radians((360 / 365.25) * (day_of_year - 80))))
    chi = math.acos(
        math.sin(delta) * math.sin(latitude)
        + math.cos(delta) * math.cos(latitude) * math.cos(alpha)
    )
    tan_xi = math.sin(alpha) / (
        math.sin(latitude) * math.cos(alpha) - math.cos(latitude) * math.tan(delta)
    )

    if alpha > 0 and tan_xi > 0:
        xi = math.pi + math.atan(tan_xi)
    elif alpha > 0 and tan_xi < 0:
        xi = 2 * math.pi + math.atan(tan_xi)
    elif alpha < 0 and tan_xi > 0:
        xi = math.atan(tan_xi)
    else:
        xi = math.pi + math.atan(tan_xi)
    ...
    P_theo = p0 * a_eff * (
        math.cos(epsilon) * math.cos(chi)
        + math.sin(epsilon) * math.sin(chi) * math.cos(xi - zeta)
    )
...
def inputs_for_rel_op(
    ...
    sec_correction = int((correction - min_correction) * 60)  # Local time delay in seconds
    ...
    is_leap_year = (
        (date_and_time.year % 4 == 0)
        and (date_and_time.year % 100 != 0 or date_and_time.year % 400 == 0)
    )
    if is_leap_year:
        months[1] = 29
    ...
    dst_start_day = sum(months[:2]) + calendar.monthcalendar(
        date_and_time.year, date_and_time.month
    )[1][6]
    # which day of year is the 1st Sunday of Nov in that year
    dst_end_day = sum(months[:10]) + calendar.monthcalendar(
        date_and_time.year, date_and_time.month
    )[0][6]
